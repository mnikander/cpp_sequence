...
#include "../src/source/from_vector.hpp"
...
{
    using namespace seq;
    std::vector<i64> input{0, 1, 2, 4, 8};
    i64 result = 0;

    // pipeline stages, from last to first
    auto sink     = to_value(result);
    auto stage    = reduce<i64>(std::plus<i64>{}, 0L, sink);
    auto sequence = from_vector(input, stage);

    sequence.yield(5);
    EXPECT_EQ(result, 15);
}

TEST(vector, nested_pipeline)
{
    using namespace seq;
    std::vector<i64> input{0, 1, 2, 4, 8};
    i64 result = 0;
